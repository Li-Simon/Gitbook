...
&emsp;&emsp;$$\mathbf{z^{(l)}}=\mathbf{W^{(l)}a^{(l-1)} + b^{(l-1)}}$$   
...
我们去掉下表i,也就是对任意一个数据。如茶表示如下： 
&emsp;&emsp;$$E_i = \frac{1}{2} (\mathbf{y-o})^2$$   
&emsp;&emsp;&emsp;&emsp;=$$\frac{1}{2} \displaystyle \sum_{k=1}^{n_L}(y_k-o_k)^2$$
