...
XGBoost模型是基于GBDT模型的改进提高版本，它使用的是CART树，对GBDT提出四方面的改进：  
1. 利用函数的二阶导数来替代GBDT中的一阶导，是算法更快收敛  
$$\kern{4 em} L(\Phi) = \displaystyle \sum_i l(\hat y_i,y_i) + \displaystyle \sum_k \Omega(f_k)$$  
其中：  $$\Omega(f) = y\gamma + \frac{1}{2}\lambda||\omega||^2$$  
$$\kern{4 em}  L^t = \displaystyle \sum_{i=1}^nl(y_i,\hat y_{i-1}+f_t(x_i)) + \displaystyle \sum_k \Omega(f_t)$$  
$$\kern{4 em}  L^t \approx \displaystyle \sum_{i=1}^nl(y_i,\hat y^{(t-1)}) + g_if_t(x_i) + \frac{1}{2}h_if_t^2(x_i) + \Omega(f_t)$$  
where: $$g_i = \partial_{\hat y} l(y_i, \hat y^{(t-1)})$$  
$$\kern{4 em} h_i = \partial_{\hat y}^2 l(y_i, \hat y^{(t-1)})$$  
$$\kern{4 em} => L^t \approx \displaystyle\sum_{i=1}^n[g_if_t(x_i) + \frac{1}{2}h_if_t^2(x_i)] + \Omega(f_t)$$  
2. 模型的约束\(叶子节点，树的深度等\)  
3. 自动寻出最佳裂项节点  
4. 类别变量的处理
...
1. bagging:基函数bias小，但是variance大，也就是比较复杂的模型。
2. boosting:目的是通过提升方法来降低bias，因此要选择variance很小的基函数，也就是较浅的树。最简单的就是树桩。

#### Random Forest

RF和bagging 不同的是bagging只使用bootstrap有放回的采样样本，但随机森林即随机采样样本，也随机选择特征，因此防止过拟合能力更强，降低方差。
