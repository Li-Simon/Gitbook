...
Then we can write:  
$$\mathbf{A(x + {\Delta}x) = b + {\Delta}b}$$  
or $$\mathbf{Ax + A{\Delta}x = b + {\Delta}b}$$  
But because $$\mathbf{Ax = b}$$, it follows that   
$$\mathbf{A{\Delta}x = {\Delta}b}$$  
So：
$$\mathbf{\delta x=A^{-1}{\Delta}b}$$  
Using the matrix norm properties:  
$$\mathbf{||Ax|| \leq ||A||||x||} $$   
We can get:  
$$\mathbf{||A^{-1}{\Delta}b|| \leq ||A^{-1}||||{\Delta}b||}$$,  
Also, we can get:  
$$\mathbf{||b|| =  ||Ax|| \leq ||A||||x||}$$  
Using equations 2.5 and 2.6 we can get:  
$$\mathbf{\frac{||{\Delta}x||}{||A||\cdot||x||} \leq  \frac{||A^{-1}||\cdot||{\Delta}b||}{||b||}}$$  
Let's define condition number as: $$\mathbf{K(A)=||A||\cdot||A^{-1}||}$$  
we can rewrite equation 2.7 as:  
$$\mathbf{\frac{||{\Delta}x||}{||x||} \leq  K(A)\cdot\frac{||{\Delta}b||}{||b||}}$$
...
$$\mathbf{(A + \Delta A)(x + \Delta x) = b} $$, we can obtain:  
$$\mathbf{\frac{||{\Delta}x||}{||x + \Delta x||} \leq  K(A)\cdot\frac{||\Delta A||}{||A||}}$$
