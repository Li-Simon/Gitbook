...
比特币的交易如下：Alice转给Bob比特币的数量是X，意味着Bob有X个UTXO\(Unspent Transaction Output\)，Bob可以用这UTXO转账给Carol,那为啥Bob就能用Alice转给他的那X个比特币呢？因为交易a的输出交易中，写明了Bob的地址以及转移的比特币数量，输出交易如下：
...
1. OP\_DUP ：duplicate operator,复制操作



![](/区块链/比特币的交易脚本/import.png)
