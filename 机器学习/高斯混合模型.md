...
y是我们观测到的数据，比如硬币的正反面，或者它就是一幅图像(一个矩阵)。  
###EM算法
极大化对数似然函数。  
完全数据的似然函数是：  
&emsp;&emsp;$$P(y,\gamma|\theta) = \displaystyle \prod _{j=1}^NP(y_j,\gamma_{j1},\gamma_{j2},...,\gamma_{jK}|\theta)$$  
...
&emsp;&emsp;$$ \log P(y,\gamma|\theta) = \displaystyle \prod _{k=1}^K\alpha_k^{n_k}\displaystyle \prod _{j=1}^N[\phi(y_j|\theta_k)]^{\gamma_{jk}}$$  
式中$$\displaystyle \sum _{j=1}^N\gamma_{jk} = n_k,\displaystyle \sum _{k=1}^Kn_k = N$$。   
