...
当输入空间的划分确定时，可以用平方误差$$\sum _{x_i \in R_m} (y_i - f(x_i))^2$$来表示回归书对于训练数据的预测误差，用平方误差最小的准则来求解每个单元上的最优输出值。
可以求得
...
$$min_{j,s}[\sum_{x_i \in R_1}(y_i - c_1)^2 + \sum_{x_i \in R_2}(y_i - c_2)^2]$$,找到是的损失最小的(j,s)对即可，也就是说找到最优特征$$j^*$$，并找到最优特征的分割值$$s^*$$s。递归执行(j,s)的选择过程，知道满足停止条件为止。
...
3.对子节点递归调用1)，2)步骤，直到满足停止条件
4.返回CART分类树T

###CART总结
CART(classification and regression tree)即分类回归树，意思是它既可以用来做分类页可以用来做回归，对分类问题，我们基于基尼系数构造分类树，对于回归问题，我们基于平方误差最小，构造回归树。
