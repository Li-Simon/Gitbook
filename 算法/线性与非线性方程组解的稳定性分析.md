...
&emsp;&emsp;$$\mathbf{A}$$. Consider $$\mathbf{A}$$ is changed to $$\mathbf{A+\Delta{A}}$$ and the solution changes from $$\mathbf{x}$$ to $$\mathbf{x+\Delta{x}}$$  
&emsp;&emsp;$$ \mathbf{(A + \Delta A)(x + \Delta x) = b} $$, we can obtain:  
