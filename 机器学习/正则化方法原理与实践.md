...
$$\kern{4 em}RR(\lambda) =(\mathbf{y} - \mathbf{X\beta})^T(\mathbf{y} - \mathbf{X\beta}) + \lambda \beta^T\beta $$  
...
对于L2正则化的最小二乘法：  
 ...
 从上面的分式$$\frac{d_j^2}{d_j^2 + \lambda}$$可以知道，对于$$d_j \ll \lambda$$,则相应的方向的作用收缩到0。可以认为L2就是对数据进行了SVD分解后，之保留了$$d_j \gt \lambda$$的分量。 
