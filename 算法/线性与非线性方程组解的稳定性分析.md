...
[Example 1](http://engrwww.usask.ca/classes/EE/840/notes/ILL_Conditioned%20Systems.pdf):
...
$$\mathbf{\hat{\beta} = argmin_{\beta}S(\beta)=argmin_{\beta}\sum_{i = 1}^{m}[y_i - f(x_i,\beta)]^2}\tag{4.1}$$
$$y_i$$是观测值，$$x_i$$是已知变量，一共m组观测值。
当$$\mathbf{f(x,\beta)}$$是线性方程组时，即 $$\mathbf{f(X,\beta) = A\beta}$$
cost function 可以表示如下：
$$\mathbf{L(\beta) = ||Y - A\beta||^2}\tag{4.2}$$
其中$$\mathbf{Y= [y_1, y_2...y_m]^T}\tag{4.3}$$
cost function 对$$\mathbf{\beta}$$
$$\mathbf{L(\beta) = Y^TY - 2Y^TA\beta + {\beta}^TA^TA\beta}\tag{4.4}$$
求导，
$$-2A^TY + 2 (A^TA)\beta = 0\tag{4.5}$$
再让导数等于0,就可以求得解为：$$\mathbf{\beta}$$
$$\mathbf{\beta = (A^TA)^{-1}A^TY}\tag{4.6}$$
我们接下来主要讨论非线性的情况：
也就是残差$$\mathbf{y_i - f(x_i,\beta)}$$不能表示成线性形式。
当然我们有很多方法来求解方程(4.1),比如[梯度下降法](https://en.wikipedia.org/wiki/Gradient_descent)，[牛顿法](https://en.wikipedia.org/wiki/Newton%27s_method_in_optimization)，[高斯-牛顿法](https://en.wikipedia.org/wiki/Gauss%E2%80%93Newton_algorithm)，以及[Levenberg-Marquardt](https://en.wikipedia.org/wiki/Levenberg%E2%80%93Marquardt_algorithm)
[梯度下降法](https://en.wikipedia.org/wiki/Gradient_descent)
把损失函数展开到一阶。
$$f(x + \alpha \overrightarrow d) = f(x_0) + \alpha f'(x_0)\overrightarrow d + O({\alpha}^2)\tag{4.A}$$
$$x_{t+1} = x_t -\alpha f'(x_t)\tag{4.B}$$,

[牛顿法](https://en.wikipedia.org/wiki/Newton%27s_method_in_optimization)
把cost function进行展开到二阶：
$$f(x_{t+1}) = f(x_t) + g(x_{t+1} - x_t)  + \frac{1}{2} (x_{t+1} -x_t)^TH(x_{t+1} -x_t)\tag{4.7}$$
求导，$$\frac{\partial f}{\partial x_t} = g + H(x_{t+1} -x_t)$$,让导数为0就有
$$x_{t+1} = x_t - H^{-1}g\tag{4.8}$$
要是H是正定的，上面的就是凸函数，也就一定有了最小值。可惜H不一定是正定的，这就引导出了下面的方法

[高斯-牛顿法](https://en.wikipedia.org/wiki/Gauss%E2%80%93Newton_algorithm)
cost function可以表示成残差的形式：
$$L(x) = \sum_{i=1}^{m}r_i(x)^2\tag{4.9}$$
根据牛顿法：
$$x_{t+1} = x_t - H^{-1}g\tag{4.10}$$,
梯度表示为：
$$g_j=2 \sum _i r_i \frac{\partial r_i}{\partial x_j}\tag{4.11}$$
对方程(4.9)求二阶导，我们可以得到Hessian矩阵：
$$H_{jk} = 2\sum _i (\frac{\partial r_i}{\partial x_j}\frac{\partial r_i}{\partial x_k} + r_i\frac{\partial ^2 r_i}{\partial x_j \partial x_k})\tag{4.12}$$,
当残差$$r_i$$很小的时候,我们就可以去掉最后一项，因此
$$H_{jk} \approx  2 \sum _i J_{ij}J_{ik} \quad With \quad J_{ij} = \frac{\partial r_i}{\partial x_j}\tag{4.13}$$
这样Hessian就是半正定了。
因此参数迭代公式(4.10)可以写成：
$$x_{t+1} = x_t - (J^TJ)^{-1}J^Tr\tag{4.14}$$,
...
$$L(\beta) = \sum_{i=1}^{m}[y_i - f(x_i,\beta)]^2 + \lambda||\beta||^2\tag{5.1}$$
可以表示成：
$$L(\beta) = L(\beta_0) + (\beta - \beta_0)^T\nabla_\beta L(\beta_0) + \frac{1}{2}(\beta - \beta_0)^TH(\beta - \beta_0)+ \lambda||\beta||^2+O(\beta^3)\tag{5.2}$$
求一阶导数：
$$L(\beta) = L(\beta_0) + (\beta - \beta_0)^Tg + \frac{1}{2}(\beta - \beta_0)^TH(\beta - \beta_0)+ \frac{1}{2}\lambda||\beta||^2+O(\beta^3)\tag{5.2}$$
令其为0：
$$\frac{\partial L(\beta)}{\partial \beta} = g + H(\beta - \beta_0) + \lambda\beta = 0\tag{4.11}$$
就得到：
$$\beta = (H + \lambda I_n)^{-1}H\beta_0 - (H + \lambda I_n)^{-1}g\tag{4.15}$$,
