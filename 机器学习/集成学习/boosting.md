...
$$\kern{4 em} = \displaystyle \sum_{m=1}^M[\gamma T_m + \frac{1}{2}\lambda ||w_m||^2_2 + \alpha ||w_m||_1 ]$$  
...
$$\kern{4 em}J_m(\phi_m) = \displaystyle \sum_{i=1}^nL(y_i, \hat f^{(m-1)(x_i)}+\phi_m(x_i))$$  
对于梯度提升算法，基函数是如下的树：  
$$\kern{4 em} \phi_m(x) = \displaystyle \sum_{j=1}^Tw_{jm}I(x \in R_{jm})$$  
这里的T是第m颗树叶子节点的个数，$$w_jm$$是第m颗树中第j个叶子节点的权重。  
对上式进行二阶泰勒展开,并忽略掉常数项可以得到：  
$$\kern{4 em}J_m(\phi_m) = \displaystyle \sum_{i=1}^n[\hat g_m(x_i)\phi_m(x_i) +\frac{1}{2}\hat h_m(x_i)\phi_m(x_i)^2]$$  
代入：  
$$\kern{4 em} \phi_m(x) = \displaystyle \sum_{j=1}^Tw_{jm}I(x \in R_{jm})$$  
$$\kern{4 em}J_m(\phi_m) = \displaystyle \sum_{i=1}^n[\hat g_m(x_i)\displaystyle \sum_{j=1}^Tw_{jm}I(x \in R_{jm}) +\frac{1}{2}\hat h_m(x_i)(\displaystyle \sum_{j=1}^Tw_{jm}I(x \in R_{jm}))^2]$$  
由于每个样本点只能属于一个叶子节点，因此：  
$$\kern{4 em}I(x \in R_{jm})I(x \in R_{im}) = \delta_{ij}$$  
因此上面可以简化成：  
$$\kern{4 em}J_m(\phi_m) = \displaystyle \sum_{i=1}^n[\hat g_m(x_i)\displaystyle \sum_{j=1}^Tw_{jm}I(x \in R_{jm}) +\frac{1}{2}\hat h_m(x_i)\displaystyle \sum_{j=1}^Tw_{jm}^2I(x \in R_{jm})]$$  
$$\kern{4 em} = \displaystyle \sum_{j=1}^T\displaystyle \sum_{i\in I_{jm}}[\hat g_m(x_i)w_{jm} + \frac{1}{2}\hat h_m(x_i)w_{jm}^2]$$  
定义：  
$$\kern{4 em} G_{jm} = \displaystyle  \sum_{i \in I_{jm}} \hat g_m(x_i)$$  
$$\kern{4 em} H_{jm} = \displaystyle  \sum_{i \in I_{jm}} \hat h_m(x_i)$$  
因此，可以把cost function写成：  
$$J_m(\phi_m) = \displaystyle \sum_{j=1}^T[G_{jm}w_{jm} + \frac{1}{2}H_{jm}w_{jm}^2]$$  
$$\kern{4 em} \ge -\displaystyle \sum_{j=1}^T\frac{1}{2}\frac{G_{jm}^2}{H_{jm}}$$  
成立条件是：  
$$w_{jm} = -\frac{G_{jm}}{H_{jm}}, j=1,2...,T$$  
