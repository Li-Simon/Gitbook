...
$$w_{m+1,i} = \frac{w_{mi}}{Z_m}exp(-\alpha _m y_i G_m(x_i)), i =1, 2,...,N$$   
这里，$$Z_m$$是归一化因子： 
$$Z_m =\displaystyle\sum_{i=1}^N w_{mi}exp(-\alpha _m y_i G_m(x_i))$$  
它使$$D_{m+1}$$成为一个概率分布。  
(3)构建基本分类器的线性组合：  
$$f(x) = \displaystyle\sum_{i=1}^N \alpha _m G_m(x)$$  
得到最终分类器   
$$\space \qquad G(x) = sign(f(x)) = sign(\displaystyle\sum_{i=1}^N \alpha _m G_m(x))$$
