...
5. Gini系数
   ...
   &emsp;&emsp;&emsp;$$S = k_B \ln \Omega =-k_B\sum _i p_i \ln p_i $$  
   ...
   &emsp;&emsp;$$g_R(D,A) = \frac{g(D,A)}{H(A}$$。  
   ...
   &emsp;&emsp;$$Gini(p) = \sum_{i}^{i=n} p_i(1 - p_i) = 1 - \sum_{i=1}^{i=K} p_i^2$$  
   ...
   &emsp;&emsp;$$Gini(p) = 1 - \sum_{i=1}^{i=K} (\frac{|C_k|}{|D|})^2$$
...
&emsp;&emsp;$$C(T) = \sum_{t=1}^{t=|T|} N_t H_t$$  
其中$$N_t$$是叶子节点t上的样本点数目，$$H_t$$是叶子节点t的熵。  
叶子节点的熵定义为：  
&emsp;&emsp;$$H_t = -\sum_{k=1}^{k=K}\frac{N_{tk}}{N_t}\ln \frac{N_{tk}}{N_t}$$,  
...
&emsp;&emsp;$$C_{\alpha}(T) = C(T) + \alpha |T|$$  
...
&emsp;&emsp;$$C_{\alpha}(T_{A}) \le C_{\alpha}(T_{B})$$  
则进行剪枝，将父节点变成新的叶节点。  
\(3\)返回\(2\),直到不能继续为止，得到损失函数最小的子树$$T_{\alpha}$$

#### 局部加权平均

&emsp;&emsp;$$L(\theta) = \frac{1}{N}\sum _i w_i(\hat y_i - y_i)^2$$  
...
&emsp;&emsp;$$D = ((x_1, y_1),(x_2, y_2)...(x_N, y_N))$$.  
假设已将输入空间划分为M个单元$$R_1, R_2,...,R_M$$,并且每个单元$$R_m$$上有一个固定的输出值$$c_m$$,于是回归树模型可以表示为：  
&emsp;&emsp;$$f(x) = \sum_{m=1}^{m=M} c_m I(x \in R_m)$$  
当输入空间的划分确定时，可以用平方误差  
&emsp;&emsp;$$\sum _{x_i \in R_m} (y_i - f(x_i))^2$$  
来表示回归书对于训练数据的预测误差，用平方误差最小的准则来求解每个单元上的最优输出值。  
可以求得  
&emsp;&emsp;$$\hat c_m = ave(y_i | x_i \in R_m)$$。  
问题是，怎么对输入的空间进行划分？  
回归树可以处理离散特征与连续特征，对于连续特征，若这里按第j个特征的取值s进行划分，切分后的两个区域分别为：  
&emsp;&emsp;$$R_1(j,s) = (x_i|x_i^j \le s)$$  
&emsp;&emsp;$$R_2(j,s) = (x_i|x_i^j \gt s)$$  
若为离散特征，则找到第j个特征下的取值s:  
&emsp;&emsp;$$R_1(j,s) = (x_i|x_i^j = s)$$  
&emsp;&emsp;$$R_2(j,s) = (x_i|x_i^j \ne s)$$  
分别计算$$R_1, R_2$$的类别估计$$c_1, c_2$$,然后计算按照\(j,s\)切分后的损失:  
&emsp;&emsp;$$min_{j,s}[\sum_{x_i \in R_1}(y_i - c_1)^2 + \sum_{x_i \in R_2}(y_i - c_2)^2]$$,找到是的损失最小的\(j,s\)对即可，也就是说找到最优特征$$j^*$$，并找到最优特征的分割值$$s^*$$s。递归执行\(j,s\)的选择过程，知道满足停止条件为止。递归的意思是，对分割出的两个区域$$R_1, R_2$$分别进行如上的步骤，再分割下去。
...
&emsp;&emsp;$$R_2(j,s) = (x_i|x_i^j \gt s)$$  
其中：  
&emsp;&emsp;$$c_1 = \frac{1}{N_1}\sum_{x_i \in R_1}y_i$$  
&emsp;&emsp;$$c_2 = \frac{1}{N_2}\sum_{x_i \in R_2}y_i$$  
2.遍历所有的可能解\(j,s\),找到最优的$$(j^*,s^*)$$,最优解使得如下损失函数最小，  
&emsp;&emsp;$$min_{j,s}[\sum_{x_i \in R_1}(y_i - c_1)^2 + \sum_{x_i \in R_2}(y_i - c_2)^2]$$  
按照最优特征$$(j^*,s^*)$$来切分即可  
3.递归调用1.，2.步骤，直到满足停止条件  
4.将输入空间划分为M个区域$$R_1, R_2,...,R_M$$,返回决策树T  
&emsp;&emsp;$$f(x) = \sum_{m=1}^{m=M} \hat c_m I(x \in R_m)$$
...
&emsp;&emsp;$$Gini(D,A) = \frac{|D_1|}{D}Gini(D_1) + \frac{|D_2|}{D}Gini(D_2)$$  其中  
&emsp;&emsp;$$D_1(A,a) = (x_i|x_i^A = a)$$  
&emsp;&emsp;$$D_2(A,a) = (x_i|x_i^A \ne a)$$  
...
&emsp;&emsp;$$C_{\alpha}(t) = C(t) + \alpha $$  
  以t为根结点的字数$$T_t$$的损失函数是\(不进行剪枝时的损失函数\)：  
&emsp;&emsp;$$C_{\alpha}(T_t) = C(T_t) + \alpha |T_t| $$  
当\alpha 小于某个值时，不剪枝会使得整体的损失函数较小，也就是：  
&emsp;&emsp;$$C_{\alpha}(T_t) \lt C_{\alpha}(t)$$  
当$$\alpha$$增大时，在某一$$\alpha$$有：  
&emsp;&emsp;$$C_{\alpha}(T_t) = C_{\alpha}(t)$$  
当$$\alpha$$再增大时，不等式反向，只要$$\alpha = \frac{C(t) - C(T_t)}{|T_t|-1}$$, $$T_t$$与t有相同的损失函数。  
  为此， 对$$T_0$$中每一个内部节点t,计算：  
&emsp;&emsp;$$g(t) = \frac{C(t) - C(T_t)}{|T_t|-1}$$  
