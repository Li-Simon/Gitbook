...
第四行$$c_t$$是cell state,由上一时刻的$$c_{t-1}$$和输入得到,两者相互独立。如果遗忘门$$f_t$$取0的话，也就是调节参数$$W_f, U_f$$使得对于任意的输入$$x_t$$与上一次的输出$$h_{t-1}$$都有$$f_t$$趋于0，那么上一时刻的状态就会全部被清空，然后只关注此时刻的输入，这就是传统的RNN。输入门$$i_t$$决定是否接收此时刻的输入。  最后输出门$$o_t$$决定是否输出cell state。  
