# 线性与非线性方程组解的稳定性分析
最速下降法中，Hessian矩阵的条件数决定了系统的收敛速度。  
# 线性方程组解的稳定性分析
...
$$\kern{4 em} \mathbf{\hat{\beta} = argmin_{\beta}S(\beta)=argmin_{\beta}\sum_{i = 1}^{m}[y_i - f(x_i,\beta)]^2}$$  
...
$$\kern{4 em} \mathbf{L(\beta) = ||Y - A\beta||^2}$$  
其中$$\mathbf{Y= [y_1, y_2...y_m]^T}$$  
cost function 对$$\mathbf{\beta}$$  
$$\kern{4 em} \mathbf{L(\beta) = Y^TY - 2Y^TA\beta + {\beta}^TA^TA\beta}$$  
求导，  
$$\kern{4 em} -2A^TY + 2 (A^TA)\beta = 0$$  
再让导数等于0,就可以求得解为：$$\mathbf{\beta}$$  
$$\kern{4 em} \mathbf{\beta = (A^TA)^{-1}A^TY}$$
...
$$\kern{4 em} x_{t+1} = x_t -\alpha f'(x_t)$$,
