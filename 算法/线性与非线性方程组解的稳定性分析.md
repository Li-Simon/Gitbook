...
Then we can write:  
$$\mathbf{A(x + {\triangle}x) = b + {\triangle}b}$$  
or $$\mathbf{Ax + A{\triangle}x = b + {\triangle}b}$$  
But because $$\mathbf{Ax = b}$$, it follows that   
$$\mathbf{A{\triangle}x = {\triangle}b}$$  
So：
$$\mathbf{{\triangle}x=A^{-1}{\triangle}b}$$  
Using the matrix norm properties:  
$$\mathbf{||Ax|| \leq ||A||||x||} $$   
We can get:  
$$\mathbf{||A^{-1}{\triangle}b|| \leq ||A^{-1}||||{\triangle}b||}$$,  
Also, we can get:  
$$\mathbf{||b|| =  ||Ax|| \leq ||A||||x||}$$  
Using equations 2.5 and 2.6 we can get:  
$$\mathbf{\frac{||{\triangle}x||}{||A||\cdot||x||} \leq  \frac{||A^{-1}||\cdot||{\triangle}b||}{||b||}}$$  
Let's define condition number as: $$\mathbf{K(A)=||A||\cdot||A^{-1}||}$$  
we can rewrite equation 2.7 as:  
$$\mathbf{\frac{||{\triangle}x||}{||x||} \leq  K(A)\cdot\frac{{||\triangle}b||}{||b||}}$$
...
$$\mathbf{(A + \triangle A)(x + \triangle x) = b} $$, we can obtain:  
$$\mathbf{\frac{||{\triangle}x||}{||x + \triangle x||} \leq  K(A)\cdot\frac{{||\triangle}A||}{||A||}}$$
...
$$\mathbf{\hat{\beta} = argmin_{\beta}S(\beta)=argmin_{\beta}\sum_{i = 1}^{m}[y_i - f(x_i,\beta)]^2}$$  
...
$$\mathbf{L(\beta) = ||Y - A\beta||^2}$$  
其中$$\mathbf{Y= [y_1, y_2...y_m]^T}$$  
cost function 对$$\mathbf{\beta}$$  
$$\mathbf{L(\beta) = Y^TY - 2Y^TA\beta + {\beta}^TA^TA\beta}$$  
求导，  
$$-2A^TY + 2 (A^TA)\beta = 0$$  
再让导数等于0,就可以求得解为：$$\mathbf{\beta}$$  
$$\mathbf{\beta = (A^TA)^{-1}A^TY}$$
