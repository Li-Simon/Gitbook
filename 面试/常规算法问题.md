...
$$\kern{4 em} f[i] = f[i-1] + d[i]$$，if  f[i-1] > 0
$$\kern{4 em} f[i] =d[i]$$，if  f[i-1] $$\le$$ 0
![](/assets/max_value_continue_subarray.png)


#### 从1到整数n中1出现的次数。
思路：通过最高是1还是大于1来操作。以3154为例
如果最高位是3大于1,我们先计算1-3000之间1的数目。再计算3001-3154.   
1-3000之间1的数目是千位含有1000-1999有$$10^3$$,还有当千位为0，1，2时，个位，十位，百位随机挑选一个为1，其它两位任意，一共有firstNum*(lenght-1)*pow(10,length-2) 个组合；
$$firstNum>1, numberOf1 = (firstNum)*(length - 1)*pow(10,length-2) + pow(10,length-1);
$$  
$$firstNum==1,numberOf1 = (length - 1)*pow(10,length-2) + residuleNum + 1;$$  
此时，高位为1，则余数+1个数字最高位含1.再加上低位含有1的情况，也就是有length-1位可选为1，其它lenght-2位任意。再把余数residuleNum 返回继续计算，因为residuleNum 相当于不计算最高位为1之后，从pow(10,length -1)到n含1的个数，也就是1-residuleNum含1的数目。  

```cpp
int CMath::NumberOf1InN(int n)
{
	if(n<10)
	{
		if(n >= 1)
	...
	int residuleNum = n%(pow(10,length-1));
	int numberOf1 = 0;
	if(firstNum > 1)
	{
		numberOf1 = (firstNum)*(length - 1)*pow(10,length-2) + pow(10,length-1);
	}
	else
	{
		numberOf1 = (length - 1)*pow(10,length-2) + residuleNum + 1;
	}
	return numberOf1 + NumberOf1InN(residuleNum);
}

int CMath::pow(int base, int n)
{
	int result = 1;
	if(n==0)
		return 1;
...
####把数组排成最小的数
输入一个正整数数组，把它拼接起来排成一个数，使得这个数最小。比如输入的是{3,32,321}则拼成的最小数是321323.  
其实就是字典排序，把这些数字看成字符，然后像字典一样再排序，排在前面的放在前面来保证拼接的数最小。  
通过调用STL algorithm中的qsort来对字符排序，自己定义compare函数，实际也是调用strcmp来排序。
```cpp
...
void CMath::PrintMinNUmber(int* numbers, int length)
{
