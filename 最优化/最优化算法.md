...
$$\kern{16 em}h(\theta) = \sum^{n}_{j = 0}\theta_{k}x_{j} $$  
$$\kern{16 em}J(\theta) = \frac{1}{2m}\sum_{i = 0}^{m}(y^i - h_{\theta}(x^i))^2 $$  
//n是特征的个数, m的训练样本的数目

# 1\)批量梯度下降法（Batch Gradient Descent, BDG）

\(1\)将$$J(\theta)$$对$$\theta$$求偏导，得到每个$$\theta$$对应的梯度：  
$$\kern{16 em}\frac{\partial J(\theta)}{\partial \theta_{j}} = -\frac{1}{m}\sum_{i=1}^m(y^i - h_{\theta}(x^i))x_{j}^{i}$$  
\(2\)由于要最小化风险函数，所以按照每个参数$$\theta$$的负梯度方向来更新$$\theta$$  
$$\kern{16 em}\theta_{j}' = \theta{j} - \frac{\partial J(\theta)}{\partial \theta_{j}}  \theta{j} + \frac{1}{m}\sum_{i=1}^m(y^i - h_{\theta}(x^i))x_{j}^{i}$$
...
$$\kern{8 em}= \frac{1}{m}\sum_{i=1}^{m}cost(\theta,(x^i,y^i))$$  
$$cost(\theta,(x^i,y^i)) = \frac{1}{2}(y^i - h_{\theta}(x^i))^2$$  
\(2\)每个样本的损失函数，对$$\theta$$求偏导得到对应的梯度，来更新$$\theta$$  
$$\kern{16 em}\theta_{j}' = \theta_{j} + (y^i - h_{\theta}(x^i))x_{j}^{i}$$  
...
左乘$$\mathbf{P^T_{k}}$$：  
$$\mathbf{P^T_{k}Ax_*} = \sum_{i=1}^{n}{\alpha}_iP^T_{k}\mathbf{AP_i} $$  
...
$$\mathbf{f(x) = \frac{1}{2}x^TAx - x^Tb}$$,  
...
$$g'({\alpha}_k = 0)$$  
因此：  
$$\mathbf{f(x_{k+1}) = f(x_k + {\alpha}_kp_k) = :g({\alpha}_k)}$$,  
$$\mathbf{\alpha _k= \frac{p_k^Tb}{p_k^TAp_k} = \frac{p_k^T(r_k + Ax_k)}{p_k^TAp_k} = \frac{p_k^Tr_k}{p_k^TAp_k}}$$  
算法：  
$$\mathbf{r_0 := b - Ax_0}$$  
$$\mathbf{p_0 := r_0}$$  
$$\mathbf{k := 0}$$  
...
把损失函数展开到一阶。  
$$f(x + \alpha \overrightarrow d) = f(x_0) + \alpha f'(x_0)\overrightarrow d + O({\alpha}^2)$$  
...
此时，Jacobia变成  
$$H_{jk} \approx  2 \sum _i J_{ij}J_{ik} \quad With \quad J_{ij} = y_i \frac{\partial r_i}{\partial x_j}$$  
...
求一阶导数：  
$$L(\beta) = L(\beta_0) + (\beta - \beta_0)^Tg + \frac{1}{2}(\beta - \beta_0)^TH(\beta - \beta_0)+ \frac{1}{2}\lambda||\beta||^2+O(\beta^3)$$  
令其为0：  
