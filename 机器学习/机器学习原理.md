...
Hinge损失  
指数损失      
平方损失等价于误差服从高斯分布的极大似然估计。  
麦哈顿距离损失等价于误差服从拉普拉斯分布的极大似然估计。
0-1损失相当于误差分从  
数据的误差的先验分布(统计学派观点？)：  极大似然
模型参数的先验分布(贝叶斯观点)：正则化是模型复杂度的惩罚函数，你可以自己定义模型的复杂度？问题是怎么定义模型复杂度？(正则化是要求模型参数只能在0附近，但是现实可能是，真实的模型参数不在0附件，贝叶斯观点并不能与模型参数误差的先验分布联系起来，因为正则化假定模型参数的均值皆为0，但是贝叶斯观点并不要求这样。)    
