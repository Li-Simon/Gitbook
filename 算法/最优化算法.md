...
$$h(\theta) = \sum^{n}_{j = 0}\theta_{k}x_{j}  \tag{Grad.1}$$  
$$J(\theta) = \frac{1}{2m}\sum_{i = 0}^{m}(y^i - h_{\theta}(x^i))^2 \tag{Grad.2}$$  
//n是特征的个数, m的训练样本的数目

# 1\)批量梯度下降法（Batch Gradient Descent, BDG）

\(1\)将$$J(\theta)$$对$$\theta$$求偏导，得到每个$$\theta$$对应的梯度：  
$$\frac{\partial J(\theta)}{\partial \theta_{j}} = -\frac{1}{m}\sum_{i=1}^m(y^i - h_{\theta}(x^i))x_{j}^{i}\tag{Grad.3}$$  
\(2\)由于要最小化风险函数，所以按照每个参数$$\theta$$的负梯度方向来更新$$\theta$$  
$$\theta_{j}' = \theta{j} - \frac{\partial J(\theta)}{\partial \theta_{j}} = \theta{j} + \frac{1}{m}\sum_{i=1}^m(y^i - h_{\theta}(x^i))x_{j}^{i}\tag{Grad.4}$$
...
\frac{1}{m}\sum_{i=1}^{m}cost(\theta,(x^i,y^i))\tag{Grad.5}$$  
$$cost(\theta,(x^i,y^i)) = \frac{1}{2}(y^i - h_{\theta}(x^i))^2\tag{Grad.6}$$  
\(2\)每个样本的损失函数，对$$\theta$$求偏导得到对应的梯度，来更新$$\theta$$  
$$\theta_{j}' = \theta_{j} + (y^i - h_{\theta}(x^i))x_{j}^{i}\tag{Grad.7}$$  
