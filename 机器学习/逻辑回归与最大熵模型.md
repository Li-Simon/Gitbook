...
$$\kern{4 em} f(w^{(k)}+\lambda _k p_k) = \displaystyle \min_{\lambda \ge 0} f(w^{(k)}+\lambda p_k)$$    
(5)置： $$w^{(k+1)} = w^{(k)} + \lambda _k p_k$$    
(6)计算$$g_{k+1} = g(w^{(k+1)})$$，若$$||g_{k+1}||< \epsilon$$，则停止计算，得$$w^* = w^{(k+1)}$$,否则按下面方式求$$B_{k+1}$$。    
$$\kern{4 em} B_{k+1} = B_{k} + \frac{y_ky_k^T}{y_k^T\delta_k} - \frac{B_k\delta_k \delta_k^TB_k^T}{\delta_k^TB_k\delta_k}$$   
其中：   
$$\kern{4 em} y_k = g_{k+1} - g_k$$    
$$\kern{4 em} \delta_k = w^{(k+1)} - w^{(k)}$$    
(7)置k=k+1,转到(3)   
