...
有约束优化问题  
$$\kern{16 em} min f(x) $$
$$\kern{16 em} s.t. h_i(x) = 0,  i=1,2...n $$  
但是我们可以通过拉格朗日乘子转化为无约束优化问题  
$$\kern{16 em}min [f(x)+\sum_{i=1}^{n}\lambda_{i}h_{i}(x)]  $$

梯度下降法  
牛顿法  
共轭梯度法  
线性搜索:先确定方向，再确定步长，典型的就是梯度下降法与牛顿法，qusai-netwon法
置信域方法：先确定步长，再找步长方向。常见的又L-M，Dog-Leg方法
...
$$\kern{8 em}J(\theta) = \frac{1}{2m}\sum_{i = 0}^{m}(y^i - h_{\theta}(x^i))^2 = 
...
\[牛顿法\]\([https://en.wikipedia.org/wiki/Newton's\_method\_in\_optimization](https://en.wikipedia.org/wiki/Newton's_method_in_optimization)\)  
...
cost function可以表示成残差的形式：  
$$L(x) = \sum_{i=1}^{m}\lambda_i r_i(x)^2 = \sum_{i=1}^{m}\frac{1}{y_i^2} (y_i - f_i(x_i))^2 \tag{4.9}$$   
此时，Jacobia变成  
$$H_{jk} \approx  2 \sum _i J_{ij}J_{ik} \quad With \quad J_{ij} = y_i \frac{\partial r_i}{\partial x_j}\tag{4.13}$$  
